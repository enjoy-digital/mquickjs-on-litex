...
"<style>"
"*{box-sizing:border-box}body{margin:0;background:#020406;color:#e8edf2;font:13px/1.45 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}"
...
"h1{font:700 24px/1.05 sans-serif;margin:0;color:#f7f9fb}h1 b,p{color:#12bdf2}p{margin:5px 0 0;text-transform:uppercase}"
".badge{min-width:94px;border:1px solid #0b87bd;color:#12bdf2;text-align:center;padding:6px 9px;border-radius:4px}.badge.on{background:#071a10;color:#8ed94f;border-color:#8ed94f}"
".grid{display:grid;grid-template-columns:minmax(0,1fr) 340px;gap:12px}.editor,.panel{background:#06080b;border:1px solid #30343a;border-radius:4px;box-shadow:0 0 0 1px #05070a inset}"
...
"aside{display:flex;flex-direction:column;gap:10px}.body{padding:10px}.bar{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.tools{grid-template-columns:repeat(2,1fr)}.slots{grid-template-columns:1fr 1fr 1fr}"
"button,select,input{border:1px solid #30343a;border-radius:4px;background:#080c10;color:#dce8f2;height:34px;padding:0 10px;font:13px sans-serif}button{cursor:pointer;background:#072234;border-color:#0b87bd;color:#e8f8ff}button.secondary{background:#0a0d11;border-color:#30343a;color:#c6d2dc}button.tool{background:#061923;border-color:#12bdf2;color:#12bdf2}button:disabled{opacity:.35;cursor:not-allowed}select{max-width:106px}"
"pre{white-space:pre-wrap;margin:0;background:#020406;border:0;padding:10px;min-height:58px;overflow:auto;color:#dce8f2}#info{min-height:146px}#log{min-height:132px;border-top:1px solid #10384d}"
...
"  demo.led(phase >> 3);\n"
"}\n"
"</textarea><div class='title'><span>console</span><span>log</span></div><pre id='log'>ready</pre></div><aside>"
"<div class='panel'><div class='title'><span>board</span><span>status</span></div><pre id='info'>detecting board...</pre></div>"
"<div class='panel'><div class='title'><span>runtime</span><span>control</span></div><div class='body bar'>"
...
"</div></div>"
"<div class='panel'><div class='title'><span>presets</span><span>scene</span></div><div class='body bar'>"
...
"</div></div>"
"<div class='panel'><div class='title'><span>peripherals</span><span>litex</span></div><div class='body bar tools'>"
...
"</div></div>"
"<div class='panel'><div class='title'><span>sdcard</span><span>slots</span></div><div class='body bar slots'>"
...
"    if (ww>220)framebuffer.text(ww-210,9,'SPD '+params.speed+' SW '+litex.getSwitches(),0x8fa3b2,1);\\n"
"  },\\n"
...
"  demo.led(phase >> 4);\\n"
"}\\n`,"
"spark:`// Cheap particle field using deterministic integer motion.\\n"
"var start = 0;\\n"
"\\n"
"function setup() {\\n"
"  start = litex.millis();\\n"
...
"  var phase = (((t - start) * params.speed) / 256) | 0;\\n"
"\\n"
"  // New heat enters from the bottom row.\\n"
"  for (var x=0;x<w;x++) heat[(h - 1) * w + x] = (x * 17 + phase * 13) & 255;\\n"
"\\n"
"  // Average neighboring heat values while moving upward.\\n"
"  for (var y=0;y<h-1;y++) {\\n"
"    for (var x=1;x<w-1;x++) {\\n"
...
"  }\\n"
"\\n"
"  var tmp = cells; cells = nextCells; nextCells = tmp;\\n"
...
"var allowed='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .:/-><=_+*()[]{};,!?&|',out='';"
"line=line.toUpperCase();"
"for(var i=0;i<line.length&&out.length<44;i++){var c=line[i];out+=allowed.indexOf(c)>=0?c:' ';}"
"return out.replace(/  +/g,' ');}"
"function previewScript(status){"
"var raw=code.value.split(/\\r?\\n/),lines=[];"
"for(var i=0;i<raw.length&&lines.length<7;i++){var s=cleanLine(raw[i]);if(s.length)lines.push(s);}"
"if(!lines.length)lines=['EMPTY SCRIPT'];"
"if(status.length>12)status=status.slice(0,12);"
"return helper+'demo.scriptPreview('+JSON.stringify(status)+','+JSON.stringify(lines)+');\\n';}"
"function preset(n){code.value=presets[n];preview('LOADED '+n.toUpperCase());}"
"async function action(n){code.value=actions[n];var r=await send(helper+code.value);if(r&&r.ok)await preview('ACTION '+n.toUpperCase(),r.text);}"
"async function postRun(src){const r=await fetch('/run',{method:'POST',headers:{'Content-Type':'text/plain'},body:src});return {ok:r.ok,text:await r.text()};}"
"async function send(src){log.textContent='running...';try{"
"const r=await postRun(src);"
"log.textContent=r.text;await refreshInfo();return r;}catch(e){log.textContent='ERR '+e;return null;}}"
"async function preview(status,prefix){log.textContent='refreshing framebuffer...';try{"
"const r=await postRun(previewScript(status));"
"log.textContent=(prefix?prefix:'')+r.text;await refreshInfo();return r;}catch(e){log.textContent='ERR '+e;return null;}}"
...
"const r=await fetch('/load?name='+encodeURIComponent(slot.value));if (!r.ok)throw await r.text();code.value=await r.text();await preview('LOADED '+slot.value,'OK loaded '+slot.value+'\\n');}"
...
"if(r.ok&&(cmd==='stop'||cmd==='reset'))await preview(cmd.toUpperCase(),text);"
"else{log.textContent=text;await refreshInfo();}}catch(e){log.textContent='ERR '+e;}}"
