 ...
 * can interact with simulated hardware (LEDs, reboot, millisecond
 * clock, busy-wait).
 *
...
static const JSClassDef js_array_buffer_class =
    JS_CLASS_DEF("ArrayBuffer", 1, js_array_buffer_constructor,
                 JS_CLASS_ARRAY_BUFFER, NULL, js_array_buffer_proto,
                 NULL, NULL);
...
static const JSClassDef js_typed_array_base_class =
    JS_CLASS_DEF("TypedArray", 0, js_typed_array_base_constructor,
                 JS_CLASS_TYPED_ARRAY, NULL, js_typed_array_base_proto,
                 NULL, NULL);
...
/* ------------------------------------------------------------------ */
/* litex — embedded hardware bindings                                 */
/* ------------------------------------------------------------------ */
...
static const JSPropDef js_framebuffer[] = {
    ...
    JS_CFUNC_DEF("line",             5, js_framebuffer_line),
    JS_CFUNC_DEF("circle",           4, js_framebuffer_circle),
    JS_CFUNC_DEF("text",             4, js_framebuffer_text),
    JS_CFUNC_DEF("fade",             1, js_framebuffer_fade),
    JS_CFUNC_DEF("scroll",           2, js_framebuffer_scroll),
...
/* Additional C function declarations (only useful for C closures). */
